...
{
    private ImageFileStatus SaveFileAsJpg(Bitmap bmp1, string saveToPath)
    {
    ...
    private static string[] CONTENTTYPELIST =
    {
        @"image/gif",
        @"image/jpeg",
        @"image/pjpeg",
    ...
    private static int MAXFILESIZE = 6 * 1024 * 1024;       // 6MB = (6 * 1024)Kb = (6 * 1024 * 1024)Bytes
    private static int MINFILESIZE = 100 * 1024;            // 100kb = (100 * 1024) Bytes
    
    public void ProcessRequest (HttpContext context)
    {
        // ImageFile is the object to return
        ...
        try
        {
            if (context.Request["merchantId"] != null &&
                Int32.TryParse(context.Request["merchantId"], out merchantId) &&
                Int32.TryParse(context.Request["minWidth"], out minWidth) &&
                Int32.TryParse(context.Request["minHeight"], out minHeight) &&
                context.Request.Files.Count > 0)
            {
                // Image size is valid, check extension
                string extension = Path.GetExtension(context.Request.Files[0].FileName).ToLower();

                if (success && !EXTENSIONLIST.Contains(extension))
                {
                    imageFile.status = (int)ImageFileStatus.InvalidExtension;
                    success = false;
                }
                    
                string contentType = context.Request.Files[0].ContentType;
                if (success && !CONTENTTYPELIST.Contains(contentType))
                {
                    imageFile.status = (int)ImageFileStatus.InvalidType;
                    success = false;
                }
                ...
                {
                    // Image not valid size
                    imageFile.status = (int)ImageFileStatus.FileSizeFailSmall;
                    success = false;
                }
                else if (fileSize > MAXFILESIZE)
                {
                    // Image not valid size
                    imageFile.status = (int)ImageFileStatus.FileSizeFailBig;
                    success = false;
                }
                ...
                if (success)
                {
                    ...
                    //Image image = Image.FromStream(context.Request.Files[0].InputStream);
                    // Get a bitmap.
                    Bitmap image = new Bitmap(context.Request.Files[0].InputStream);
                    
                    // Rotate the image using EXIF data
                    orientByExifData(ref image);

                    // Validate minimum width & height
                    if (image.Width < minWidth ||
                        image.Height < minHeight)
                    {
                        imageFile.status = (int)ImageFileStatus.BelowMinDimensions;
                    }
                    else
                    {
                        ...
                        string imageFilename = "user.jpg";// Path.GetFileNameWithoutExtension(context.Request.Files[0].FileName);
                        
                        // Get Images Url
                        string imagesUrl = System.Configuration.ConfigurationManager.AppSettings["imagesUrl"];

                        // Create the virtual path
                        string virtualPath = String.Format("/gcp/view/photo_cards/{0}/photo_{1}/",
                            merchantId,
                            uniqueId);
                        ...
                        if (!File.Exists(serverPath + imageFilename))
                        {
                            // Save the image
                            //image.Save(serverPath + imageFilename, ImageFormat.Jpeg);

                            imageFile.status = (int)SaveFileAsJpg(image, serverPath + imageFilename);
                        }
                        imageFile.resolution = image.HorizontalResolution;
                        imageFile.name = imageFilename;
                        imageFile.size = fileSize;
                        imageFile.type = "image/jpeg";
                        imageFile.url = "https://" + imagesUrl + virtualPath + imageFile.name;
                        imageFile.deleteUrl = String.Empty;
                    }
                }
            }
        }
        ...
        {
            imageFile.status = (int)ImageFileStatus.Error;
        }

        // Return the ImageFile in an array of ImageFiles (this is what the frontend code expects)
        //var filesObject = new ImageFiles();
        //filesObject.files = new[] { imageFile };

        //ImageFile[] filesObject = new ImageFile[1];
        //filesObject[0] = imageFile;
        context.Response.ContentType = "application/json";
        context.Response.Write(imageFile.ToString());
    }
    ...
    private class ImageFile
    {
        ...
        public string deleteUrl = String.Empty;
    ...
    private void orientByExifData(ref Image image)
    {
        // Rotate the image according to EXIF data
        var exif = new EXIFextractor(ref image, "n"); // got source from http://www.codeproject.com/KB/graphics/exifextractor.aspx?fid=207371
    ...
    private void orientByExifData(ref Bitmap image)
    {
        // Rotate the image according to EXIF data
        var exif = new EXIFextractor(ref image, "/n",""); // got source from http://www.codeproject.com/KB/graphics/exifextractor.aspx?fid=207371
	...
	"files":[
			{
			"name":"Colorful_nature_by_fotolympus.jpg",
			"size":330155,
			"type":"image/jpeg",
			"url":"/gcp/view/photo_cards/22/photo_523789584459f/Colorful_nature_by_fotolympus.jpg",
			"deleteUrl":"http://local.buyatab.com/gcp/server/php/?file=Colorful_nature_by_fotolympus.jpg",
			"deleteType":"DELETE"
			}
		]
	}
    ...
    /// <summary>
		...
		public class EXIFextractor : IEnumerable
		{
			...
			public void setTag(int id, int len, short type, byte [] data)
			{
				...
				this.image.SetPropertyItem( p );				
				buildDB(this.image.PropertyItems);
			}
			/// <summary>
			/// 
			/// </summary>
			...
			/// <param name="sp"></param>
			public EXIFextractor(ref Image image, string sp )
			{
			...
			string msp = "";
			public EXIFextractor(ref System.Drawing.Bitmap image, string sp, string msp )
			{
				properties = new Hashtable();
				this.sp = sp;
				this.msp = msp;
