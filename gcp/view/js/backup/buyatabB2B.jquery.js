	...
	var Template = {
					...
					$.when( self.getTemplate() ).then(function( rTemplt ){
						...
						$.when( self.appendStyle( rStyle ), self.getLanguage(), self.customLanguage(), self.getUnder() ).then(function(appStyle, rLang, cLang, rUnder){
							...
							$.when( self.styleTheme( self.settings.Style.PresetId ),
								...
								self.stylePhoto( self.settings.Merchant.PhotoCard ),
								self.styleBrowser()
		...
		generatePhotoCard: function(){
			...
			var self = this,
				...
				photoOnly = self.settings.Merchant.PhotoOnly || false;
				enabled = self.settings.Merchant.PhotoCard || false; //either ini file settings or false by default
		...
		handleEnclosure: function(){
			$(document).trigger('handleEnclosure', 'resetPhoto');
			var self = this,
				myTemplt = $(self.cTemplate),
				message = myTemplt.find('textarea#card_message').parents('.field'),
		...
		uploadPhoto: function(){
			...
			var self = this,
				...
				message,
				lang = self.Language.customize.label,
				enabled = self.settings.Merchant.PhotoCard || false,
		...
		resizePhoto: function(percent){
			...
			myImage.width( ( unit * (percent - 1) ) + 200 );
			myImage.height( 'auto' );
		...
		manipulateCart: function() {
			...
			App.Views.Cart = Backbone.View.extend({
				...
				getStorage: function(){
					if( typeof(Storage)!=="undefined" && self.PurchaseId != 6 && self.PurchaseId != 7 ){
						...
						if( lCart !== null && lCart.length > 0 && !self.settings.Merchant.PhotoOnly){
							if( self.psCard == 'null' ){
								$('html, body').animate({ scrollTop: 0 }, "fast").promise().done(function(){
									$( '<div></div>' )
			...
			App.submit = function(){
				...
				tempObj.Orientation = 'bottom'; //hardcoded for now
			...
			App.prepareEditing = function( view ){
				...
				setValidate().done(function(){
					...
					// $("select#enclosure option").filter(function() {
					// console.log(myCard.PersonalizedEnclosure.Enclosure_Id);
					// return $(this).val() == myCard.PersonalizedEnclosure.Enclosure_Id;
					// }).attr('selected', true);
