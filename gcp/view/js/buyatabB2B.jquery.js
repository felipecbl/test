	...
	var Template = {
		...
		generaIt: function(){
			...
			$.when( self.getOptions() ).then( function( rOpt ) {
				...
				self.cardsPath			= '/gcp/view/cards/' + self.chainId + '/' + self.MerchantId + '/';

				$.when(  self.getStyle(), self.getIni() ).then( function( rStyle, iniOpt ){
					...
					if(self.settings.Merchant.Language == 'fr'){
						self.underFile = 'template/fr-card.handlebars';
					}
		...
		checkFile: function(file) {
			...
			self.serverFile( customFile ).done(function(response){
				if(response.d){
					...
					self.serverFile( chainFile ).done(function(chainResponse){
						myFile = (response.d) ? chainFile : defaultFile;
					});
				}
			});

			// self.getFilePath( file ).done(function(response){
		...
		getFilePath: function( file ){
			...
			return $.ajax({
                ...
                data: "{filename:'/" + file + ", chainId: " + self.chainId + ", merchantId: " + self.MerchantId + "'}",
                contentType: "application/json; charset=utf-8",
                dataType: "json",
				async: false
            });
